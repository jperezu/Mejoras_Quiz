...
exports.randomplay = (req, res, next) => {
    req.session.vidas = req.session.vidas || 0;
    if ( req.session.vidas <= 0) {req.session.vidas = 3};
    req.session.randomPlay = req.session.randomPlay || [];
    const op = {'id': {[Sequelize.Op.notIn]: req.session.randomPlay}};
    models.quiz.count({where: op})
    .then(function(count){
        if(!count || req.session.vidas === 0){
    ...
    .then(function(quiz){
        res.render('quizzes/random_play', {quiz, score: req.session.randomPlay.length, vidas : req.session.vidas});
    });
};
...
exports.randomcheck = (req, res, next) => {
        ...
        if (result) {
            ...
            req.session.vidas --;
	    if (req.session.vidas === 0) { req.session.randomPlay = [];}
        }
